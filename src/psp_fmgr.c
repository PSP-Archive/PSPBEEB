...
psp_fmgr_getExtId(const char *szFilePath) 
{
  ...
  if((pszExt = strrchr(szFilePath, '.'))) {
    ...
    if ((!strcasecmp(pszExt, ".ssd")) || 
        (!strcasecmp(pszExt, ".dsd")) || 
        (!strcasecmp(pszExt, ".adl")) || 
        (!strcasecmp(pszExt, ".adf")) || 
        (!strcasecmp(pszExt, ".img"))) {
...
static char user_filedir_state[PSP_FMGR_MAX_NAME];
...
psp_fmgr_menu(int format, int disk_id)
{
  ...
  if (first) {
    ...
    strcpy(user_filedir_state, user_filedir_kbd);
    strcat(user_filedir_kbd, "/kbd/");
    strcat(user_filedir_set, "/set/");
    strcat(user_filedir_disk, "/discs/");
    strcat(user_filedir_state, "/state/");
  }
  ...
  if (format == FMGR_FORMAT_STATE) user_filedir = user_filedir_state;
  ...
  if (psp_file_request(user_filename, user_filedir)) {
    ...
    else 
    {
      ...
      if (file_format == FMGR_FORMAT_ZIP) {
        ...
        if (user_file_format == FMGR_FORMAT_STATE) /* State */ error = beeb_state_load(user_filename, 1);
        else
        if (user_file_format == FMGR_FORMAT_DISK) /* Disk */ error = beeb_disk_load(user_filename, disk_id, 1);
      }
      else 
      {
        if (file_format == FMGR_FORMAT_STATE) /* State */ error = beeb_state_load(user_filename, 0);
        else
        if (file_format == FMGR_FORMAT_DISK) /* Disk */ error = beeb_disk_load(user_filename, disk_id, 0);
        else
        if (file_format == FMGR_FORMAT_KBD) /* Kbd */  error = psp_kbd_load_mapping(user_filename);
        else
        if (file_format == FMGR_FORMAT_SET) /* Settings */  error = beeb_load_file_settings(user_filename);
      }
    }
