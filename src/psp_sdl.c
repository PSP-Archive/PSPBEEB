...
psp_sdl_save_thumb_png(SDL_Surface* my_surface, char* filename)
{
  ...
  for (y = 0; y < SNAP_HEIGHT; y++) {
    scan_src_pixel = src_pixel + (BEEB_WIDTH * y * 3);
...
psp_sdl_save_screenshot(void)
{
  ...
  int  screenshot_id = beeb_get_psp_screenshot_id();

  getcwd(TmpDirName, sizeof(TmpDirName));
  sprintf(TmpFileName,"%s/scr/screenshot_%d.png", TmpDirName, screenshot_id++);
  if (screenshot_id >= 10) screenshot_id = 0;
  beeb_set_psp_screenshot_id(screenshot_id);
