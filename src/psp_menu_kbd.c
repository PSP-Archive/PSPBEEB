...
psp_display_screen_kbd_menu(void)
{
  ...
  for (menu_id = 0; menu_id < MAX_MENU_KBD_ITEM; menu_id++) 
  {
        ...
        (menu_id <= MENU_KBD_JOY_RIGHT)) 
    {
      ...
      if (menu_kbd_selected == KBD_NORMAL_MAPPING  ) bbc_key = loc_kbd_mapping[kbd_id];
      else
      if (menu_kbd_selected == KBD_LTRIGGER_MAPPING) bbc_key = loc_kbd_mapping_L[kbd_id];
      else
      if (menu_kbd_selected == KBD_RTRIGGER_MAPPING) bbc_key = loc_kbd_mapping_R[kbd_id];

      if ((bbc_key >= 0) && (bbc_key < BBC_MAX_KEY)) {
        strcpy(buffer, psp_bbc_key_to_row_col[bbc_key].name);
      } else 
      if (bbc_key == KBD_UNASSIGNED) {
        sprintf(buffer, "UNASSIGNED");
      } else
      if (bbc_key == KBD_LTRIGGER_MAPPING) {
        sprintf(buffer, "L MAPPING");
      } else
      if (bbc_key == KBD_RTRIGGER_MAPPING) {
        sprintf(buffer, "R MAPPING");
      } else {
        sprintf(buffer, "KEY %d", bbc_key);
      }
...
psp_keyboard_menu_mapping(int kbd_id, int step)
{
  ...
  if (ploc_kbd_mapping[kbd_id] <  -3) ploc_kbd_mapping[kbd_id] = BBC_MAX_KEY-1;
  else
  if (ploc_kbd_mapping[kbd_id] >= BBC_MAX_KEY) ploc_kbd_mapping[kbd_id] = -3;
}
...
# ifdef USE_PSP_IRKEYB
    ...
    if (psp_irkeyb != PSP_IRKEYB_EMPTY) {
      ...
      if (psp_irkeyb == PSP_IRKEYB_RIGHT) {
        ...
        bbc_key = beeb_get_key_from_ascii(psp_irkeyb);
      }
      ...
      if (bbc_key != -1) {
            ...
            (cur_menu_id <= MENU_KBD_JOY_RIGHT)) 
        {
          ...
          if (menu_kbd_selected == -1) loc_kbd_mapping[kbd_id] = bbc_key;
          else
          if (menu_kbd_selected == KBD_LTRIGGER_MAPPING) loc_kbd_mapping_L[kbd_id] = bbc_key;
          else
          if (menu_kbd_selected == KBD_RTRIGGER_MAPPING) loc_kbd_mapping_R[kbd_id] = bbc_key;
        }
      }
    }
# endif
    ...
    if (danzeff_mode) {
      ...
      if (danzeff_key > DANZEFF_START) {
        bbc_key = beeb_get_key_from_ascii(danzeff_key);
