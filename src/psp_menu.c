...
# define MENU_CHAIN_CMD    8
# define MENU_RUN_CMD      9
# define MENU_EXEC_CMD    10
# define MENU_CAT_CMD     11
...
psp_menu_display_save_name()
{
  ...
  snprintf(buffer, 20, "Game: %s", beeb_get_save_name());
...
psp_display_screen_menu(void)
{
  ...
  for (menu_id = 0; menu_id < MAX_MENU_ITEM; menu_id++) {
    ...
    if (menu_id == MENU_SCREENSHOT) {
      sprintf(buffer,"%d", beeb_get_psp_screenshot_id());
  ...
  for (slot_id = 0; slot_id < BEEB_MAX_SAVE_STATE; slot_id++) {
    if (slot_id == cur_slot) color = PSP_MENU_SEL2_COLOR;
    else                     color = PSP_MENU_TEXT_COLOR;

    if (beeb_is_save_used(slot_id)) {
      ScePspDateTime* my_date = beeb_get_save_date(slot_id);
# ifndef LINUX_MODE
...
# endif
  ...
  if (beeb_get_save_state(cur_slot)->thumb) {
    psp_sdl_blit_thumb(289, 40, beeb_get_save_state(cur_slot)->surface);
  } else {
    psp_sdl_blit_thumb(289, 40, thumb_surface);
  }
...
psp_main_menu_reset(void)
{
  ...
  psp_sdl_back2_print(140, 160, "Reset BBC !", PSP_MENU_WARNING_COLOR);
  psp_menu_dirty = 1;
  psp_sdl_flip();
  beeb_emulator_reset();
...
psp_main_menu_run_command(void)
{
  ...
  RunName = psp_run_search(beeb_get_save_name());
  if (RunName != (char *)0) {
    snprintf(Buffer, 512, "%s", RunName);
  } else {
    RunName = beeb_get_save_name();
    snprintf(Buffer, 512, "*RUN %s", RunName);
  }
  for (Index = 0; Buffer[Index]; Index++) {
    Buffer[Index] = toupper(Buffer[Index]);
  }
  psp_kbd_run_command(Buffer);
}

static void
psp_main_menu_exec_command(void)
{
  char  Buffer[512];
  char *RunName;
  int   Index;
  ...
  if (RunName != (char *)0) {
    ...
    RunName = beeb_get_save_name();
    snprintf(Buffer, 512, "*EXEC %s", RunName);
  }
  for (Index = 0; Buffer[Index]; Index++) {
    Buffer[Index] = toupper(Buffer[Index]);
  }
...
static void
psp_main_menu_chain_command(void)
{
  char  Buffer[512];
  char *RunName;
  int   Index;
  ...
  if (RunName != (char *)0) {
    ...
    RunName = beeb_get_save_name();
    snprintf(Buffer, 512, "CHAIN\"%s\"", RunName);
  }
  for (Index = 0; Buffer[Index]; Index++) {
    Buffer[Index] = toupper(Buffer[Index]);
  }
...
static void
psp_main_menu_cat_command(void)
{
  psp_kbd_run_command("*CAT\n");
}

static void
psp_main_menu_boot_command(void)
{
...
psp_main_menu_load_slot()
{
  ...
  error = beeb_snapshot_load_slot(cur_slot);
...
{
  int error;
  error = beeb_snapshot_save_slot(cur_slot);
...
psp_main_menu_del_slot()
{
  ...
  error = beeb_snapshot_del_slot(cur_slot);
...
{
  if (step == 1) {
    cur_slot++; if (cur_slot >= BEEB_MAX_SAVE_STATE) cur_slot = 0;
  } else if (step == -1) {
    cur_slot--; if (cur_slot < 0) cur_slot = BEEB_MAX_SAVE_STATE - 1;
  }
}
        ...
        (new_pad & PSP_CTRL_LEFT  )) 
    {
      ...
      switch (cur_menu_id ) 
      {
        ...
        case MENU_LOAD_DISK0 : psp_menu_dirty = 1;
                               psp_main_menu_load(FMGR_FORMAT_DISK,0);
                               psp_main_menu_boot_command();
                               psp_menu_dirty = 1;
                               old_pad = new_pad = 0;
        break;              
        case MENU_LOAD_DISK1 : psp_menu_dirty = 1;
                               psp_main_menu_load(FMGR_FORMAT_DISK,1);
                               psp_main_menu_boot_command();
                               psp_menu_dirty = 1;
                               old_pad = new_pad = 0;
        break;              
        case MENU_CHAIN_CMD  : psp_main_menu_chain_command();
                               end_menu = 1;
        break;
        case MENU_RUN_CMD    : psp_main_menu_run_command();
                               end_menu = 1;
        break;
        case MENU_EXEC_CMD   : psp_main_menu_exec_command();
                               end_menu = 1;
        break;
        case MENU_CAT_CMD   : psp_main_menu_cat_command();
                              end_menu = 1;
        break;
