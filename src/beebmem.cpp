...
#include "include/debug.h"		//Rob added for INTON/OFF reporting only
...
//++
	...
	qINFO("Loading ROM images:");

	qWARN("FIX-ME: Need to check in ~/.beebem first..");
  getcwd(TmpPath,sizeof(TmpPath));
	strcat(TmpPath, "/config/roms.cfg");
	RomCfg=fopen(TmpPath,"rt");
...
//++
	...
	qINFO("FIX ME: You'll need to check for this in ~/.beebem first..");

  getcwd(TmpPath, sizeof(TmpPath));
	strcat(TmpPath, "/state/cmos.ram");

	pDEBUG(dL"loading CMOS file from: %s", dR, TmpPath);
//<-
  ...
  if ((CMDF3 = fopen(TmpPath,"rb"))!=NULL) {
	  ...
	  if (CMOSLength==50){
		...
		pERROR(dL"CMOS file %s is not 50 bytes long! Using default configuration instead.", dR, TmpPath);
	  }
	  fclose(CMDF3);
  }
